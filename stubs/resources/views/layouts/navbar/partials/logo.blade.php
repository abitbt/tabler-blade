{{--
    ...
    @var bool $showTitle - Show application name next to logo (default: from config)
...
@php
    $class = $class ?? '';
    $showTitle = $showTitle ?? config('tabler.logo.show_title', false);
    $smallLogo = $smallLogo ?? false;
    $url = $url ?? url('/');

    // Check for custom logos
    $customLogoSmall = config('tabler.logo.small');
    $customLogoFull = config('tabler.logo.full');
    $customLogo = $smallLogo ? $customLogoSmall : $customLogoFull;
    $useFallbackSvg = config('tabler.logo.fallback_svg', true);
...
<div class="{{ implode(' ', $brandClasses) }}">
    <a href="{{ $url }}" aria-label="{{ config('app.name') }}">
        @if ($customLogo && file_exists(public_path($customLogo)))
            {{-- Custom logo from config --}}
            <img src="{{ asset($customLogo) }}" alt="{{ config('app.name') }}" class="{{ implode(' ', $logoClasses) }}"
                @if ($smallLogo) width="32" height="32" @else height="32" @endif>
        @elseif ($useFallbackSvg)
            {{-- Fallback Tabler logo --}}
            @if ($smallLogo)
                <!-- Small Logo (32x32) -->
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 68 68" width="32" height="32"
                    class="{{ implode(' ', $logoClasses) }}">
                    <path
